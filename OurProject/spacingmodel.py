...
class SpacingModel(object):
    ...
    def add_fact(self, fact):
        ...
        if next((f for f in self.facts if f.fact_id == fact.fact_id), None):
            raise RuntimeError(
                "Error while adding fact: There is already a fact with the same ID: {}. Each fact must have a unique ID".format(
                    fact.fact_id
                )
            )
    ...
    def get_next_fact(self, current_time):
        ...
        fact_activations = [
            (f, self.calculate_activation(current_time + self.LOOKAHEAD_TIME, f))
            for f in self.facts
        ]
        ...
        if len(seen_facts) > 2:
            ...
            seen_facts = [
                (f, a)
                for (f, a) in seen_facts
                if f.fact_id != last_response.fact.fact_id
            ]
    ...
    def get_rate_of_forgetting(self, time, fact):
        ...
        for response in responses_for_fact:
            ...
            encounters = [
                encounter._replace(
                    decay=self.calculate_decay(encounter.activation, alpha)
                )
    ...
    def calculate_activation_from_encounters(self, encounters, current_time):
        ...
        return math.log(
            sum(
                [
                    math.pow((current_time - e.time) / 1000, -e.decay)
                    for e in included_encounters
                ]
            )
        )

    def calculate_predicted_reaction_time_error(
        self, test_set, decay_adjusted_encounters, reading_time
    ):
        ...
        """
        activations = [
            self.calculate_activation_from_encounters(
                decay_adjusted_encounters, e.time - 100
            )
            for e in test_set
        ]
        rt = [
            self.estimate_reaction_time_from_activation(a, reading_time)
    ...
    def get_max_reaction_time_for_fact(self, fact):
        ...
        max_rt = 1.5 * self.estimate_reaction_time_from_activation(
